class GeneratesController < ApplicationController
    def index
        ...
        while index < @subject_code_selected.count do 
            @start = Timer.find_by(time: @times_selected[index].text[0..4].to_i).id
            @finish = Timer.find_by(time: @times_selected[index].text[7..10].to_i).id
